...
const generateCSS = () =>{
    ...
    if (window.location.hostname == "www.twitch.com"){
        document.head.innerHTML = generateCSS;
        document.body.innerHTML = generateHTML;
    }
...
// Following code uses the chrome storage API to store the blocked sites and check if on or off is happening for toggle (In progress)
...
chrome.storage.local.set({blockedSites: sites}, function() {
    console.log('Blocked site list saved.');
});
...
function isSiteBlocked(site) {
    ...
    chrome.storage.sync.get(['blockedSites'], function(result){
        ...
        if (result.blockedSites.includes(site)) {
            blocked = true;
        }
    });
    return blocked;
...
window.onload = (event) => {
    ...
    let blocked = isSiteBlocked(currentSite);
    if (blocked) {
        generateCSS;
        generateHTML;
    }
